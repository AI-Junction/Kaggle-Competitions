...
selfish gene
voters
last run 4 days ago · Python notebook · 2744 views
using data from New York City Taxi Trip Duration ·
Public
...
dataDir = "C:\\Users\\echtpar\\Anaconda3\\KerasProjects\\Keras-CNN-Tutorial\\AllNewYorkTaxiTripData\\"
taxiDB = pd.read_csv(dataDir + 'train.csv')

print(taxiDB.columns.values)
taxiDB.head()
...
print(min(allLat), max(allLat))
print(min(allLong), max(allLong))
...
print(imageSize[0])
print((allLat  - latRange[0]))
print(allLatInds[:10])
print(imageSize[0]-(imageSize[0]*(allLat  - latRange[0])  / (latRange[1]  - latRange[0])).astype(int))
...
"""    
The magenta circles are sources (pickup locations), the green circles are destinations (dropoff locations) and the arrows between them are drawn in cyan. we can see an uneven distribution of sources and destinations with the periphery of Manhattan mostly serve as destination. (I'm sure some people might want to kill me for calling Brooklyn and Queens "the periphery"...)
...
We can see that around 6AM people start waking up and (most likely) heading off to work. There is a second surge of taxi rides at around 18:00/19:00 (6PM/7PM) (most likely) getting back home.
...
plt.figure(figsize=(10,15)); plt.title('Trip Distribution throughout the Year')
