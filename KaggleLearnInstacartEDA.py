# -*- coding: utf-8 -*-
"""
Created on Wed Aug  9 11:01:49 2017
...
L
forked from Instacart Simple Data Exploration by Serigne (+0/–0)
Chandrakant Pattekar
...
Here is my first Kernel where I try to explore some basic information about Instacart dataset. 
As said in the description , the Dataset is anonymized and contains a sample of over 3 million grocery orders 
from more than 200,000 Instacart users. The goal is then to predict which previously purchased products 
will be in a user’s next order. Now let's jump straight into the data and do some exploratory analysis !!
"""
...
order_products_train = pd.read_csv(path_order_products__train)
order_products_prior = pd.read_csv(path_order_products__prior)
...
"""
Firstly, let's explore order_products_train and order_products_prior files. 
These files specify which products were purchased in each order. 
More specifically, order_products__prior contains previous order contents for 
all customers and order_products_train contains the last orders for some customers only.                   
"""
...
print(order_products_all.isnull().sum())
print(order_products_all.isnull().count())
print(percent, total)
print(order_products_all.isnull().count())
...
"""For Practice start"""
grouped = order_products_all.groupby("product_id")["reordered"].aggregate({'sum'}).reset_index()
print(order_products_all.reordered.value_counts())
print(grouped[:10])
