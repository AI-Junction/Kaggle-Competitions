...
train = train.loc[:, 'smsIn'].to_frame()
train.index = pd.date_range(start = '11/1/2013', periods = len(train), freq = 'H')
...
plt.plot(train.index, train['SMSIn'])
plt.show()
...
df_date_index = train[['date','SMSIn']].set_index('date')
...
def test_stationarity(timeseries):
    ...
    dftest = sm.tsa.adfuller(timeseries['SMSIn'], autolag='AIC')
...
rolmean = df_date_index.rolling(7).mean()
#rolstd = pd.rolling_std(df_date_index, window=7)
rolstd = df_date_index.rolling(7).std()
...
#Perform Dickey-Fuller test:
print('Results of Dickey-Fuller Test:')
dftest = sm.tsa.adfuller(df_date_index['SMSIn'], autolag='AIC')
