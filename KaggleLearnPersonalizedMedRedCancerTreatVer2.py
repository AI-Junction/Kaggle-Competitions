# -*- coding: utf-8 -*-
"""
Created on Wed Mar 28 21:26:37 2018
...
def top_tfidf_feats(row, features, top_n=10):
    print("in top_tfidf_feats - row, features[20]", row, features[:20]) 
    
    topn_ids = np.argsort(row)[::-1][:top_n]
    print("topn_ids post argsort are ", topn_ids)
    
    top_feats = [(features[i], row[i]) for i in topn_ids]
    print("top_feats post argsort are ", top_feats)
    #print("top rows post argsort are ", row[i])
...
def top_mean_feats(Xtr, features, grp_ids=None, min_tfidf=0.1, top_n=10):
    ...
    print("tfidf_means length: ", len(tfidf_means))
    print("features length: ", len(features))
    print("tfidf_means: ", tfidf_means[:20])
    return top_tfidf_feats(tfidf_means, features, top_n)

def top_feats_by_class(Xtr, y, features, min_tfidf=0.1, top_n=10):
    print("in top_feats_by_class len of y: ", len(y))
    dfs = []
    labels = np.unique(y)
    print("labels = ", labels)
    for label in labels:
